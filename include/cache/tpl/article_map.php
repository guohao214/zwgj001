...
<html lang="zh-CN">
    <head>
        ...
        <![endif]-->
         ...
         <link href="/skin/default/css/map.css" rel="stylesheet">
    </head>
    <body>
        ...
        <ul class="nav navbar-nav">
            ...
            <div class="content">
                ...
                <div class="header-title">
                    <img src="/skin/default/images/map-icon_1.jpg" height="15"> <span><?php echo $type['classname'] ?></span>
                </div>
                <div class="lc_content">
                    <div class="lc_content_list">
                        <div class="map_list">
                            <h5><a href="/">首页</a></h5>
                        </div>
                        <?php $vn=0;$tablev=syClass("syModel")->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from s_classtype where  pid=0  order by orders desc,tid");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
                            <div class="map_list">
                                <h5><?php echo $v['classname'] ?></h5>
                                <ul>
                                    <?php if ($v['tid'] == 24) { ?>
                                        <?php $itemn=0;$tableitem=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,telphone,address,contact,area,sarea,shebei,weizhi,cangku_image,image_description,zhandi,jingwei,huadong_image from s_cangku a left join s_cangku_field b on (a.id=b.aid) where isshow=1 and tid in(24)  order by tid asc ");foreach($tableitem as $item){ $item["tid_leafid"]=$sy_class_type->leafid($item["tid"]);$item["n"]=$itemn=$itemn+1; $item["url"]=html_url("channel",$item,0,0,cangku); $item["title"]=stripslashes($item["title"]); $item["description"]=stripslashes($item["description"]); ?>
                                            <li><a href="<?php echo $item['url'] ?>"><?php echo $item['title'] ?></a></li>
                                        <?php } ?>
                                    <?php } else if($v['tid'] == 26) { ?>
                                        <?php $itemn=0;$tableitem=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,lc_image,image_title from s_caozuo a left join s_caozuo_field b on (a.id=b.aid) where isshow=1 and tid in(26)  order by tid asc ");foreach($tableitem as $item){ $item["tid_leafid"]=$sy_class_type->leafid($item["tid"]);$item["n"]=$itemn=$itemn+1; $item["url"]=html_url("channel",$item,0,0,caozuo); $item["title"]=stripslashes($item["title"]); $item["description"]=stripslashes($item["description"]); ?>
                                            <li><a href="<?php echo $item['url'] ?>"><?php echo $item['title'] ?></a></li>
                                        <?php } ?>
                                    <?php } else { ?>
                                        <?php $itemn=0;$tableitem=syClass("syModel")->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from s_classtype where  pid='".$v[tid]."'  order by orders desc,tid");foreach($tableitem as $item){ $item["tid_leafid"]=$sy_class_type->leafid($item["tid"]);$item["n"]=$itemn=$itemn+1; $item["classname"]=stripslashes($item["classname"]);$item["description"]=stripslashes($item["description"]); $item["url"]=html_url("classtype",$item); ?>
                                            <li><a href="<?php echo $item['url'] ?>"><?php echo $item['classname'] ?></a></li>
                                        <?php } ?>
                                    <?php } ?>
