...
<html lang="zh-CN">
    <head>
        ...
        <![endif]-->
         ...
         <link href="/skin/default/css/fzlc.css" rel="stylesheet">
    </head>
    <body>
            ...
            <div class="content">
                ...
                <div class="lc_content">
                    <div class="lc_content_list">
                        ...
                        <?php $list = array(); ?>
                        <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,content from s_fzlc a left join s_fzlc_field b on (a.id=b.aid) where isshow=1 and tid in(".$type["tid_leafid"].")  order by id asc ");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,fzlc); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                            <?php $list[] = $v ;?>
                        <?php } ?>
                        
                        <?php $count = count($list); ?>
                        <?php foreach($list as $k => $v): ?>
                            <div class="lc_lsit<?php if ($k+1 == $count ):?>_end<?php endif; ?>">
                                <div class="lc_icon"></div>
                                <div class="detail">
