...
<html lang="zh-CN">
    <head>
        ...
        <![endif]-->
         ...
         <link href="/skin/default/css/chedui.css" rel="stylesheet">
    </head>
    <body>
        ...
        <ul class="nav navbar-nav">
            ...
            <div class="content">
                ...
                <div class="header-title">
                    <img src="/skin/default/images/chedui-icon.jpg" height="15"> <span><?php echo $parent['classname'] ?></span> <img src="/skin/default/images/right-icon.jpg" height="15"> <span><?php echo $type['classname'] ?></span>
                </div>
                <div class="chedui-top">
                    <div class="chedui-top-info">
                        <div class="chedui-top-info-img">
                            <img src="<?php echo $type['litpic'] ?>">
                        </div>
                        <div class="chedui-top-info-detail">
                                <?php echo $type['body'] ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="header-title">
                    <img src="/skin/default/images/chedui-tel.jpg" height="15"><span><?php echo $type['classname'] ?>联系表</span>
                    <div class="chedui-address"><p>地址：<?php echo $type['description'] ?></p></div>
                </div>
                
                <div class="chedui-list">
                    <!--<div class="chedui-list-border">-->
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,positon,telphone,phone,qq,fax from s_busteam a left join s_busteam_field b on (a.id=b.aid) where isshow=1 and tid in(".$type["tid_leafid"].")  order by orders desc,addtime desc,id desc");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,busteam); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                                <tr>
                                    <td><?php echo $v['id'] ?></td>
                                    <td><?php echo $v['title'] ?></td>
                                    <td><?php echo $v['positon'] ?></td>
                ...
                <?php $vn=0;$tablev=syClass("syModel")->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow,body from s_classtype where  pid='".$type[tid]."'  order by orders desc,tid limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
                    <?php $bus = $v;?>
                <?php } ?>
                
                
                <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,bus_images from s_show_image a left join s_show_image_field b on (a.id=b.aid) where isshow=1 and tid in(".$bus["tid_leafid"].")  order by id asc ");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,show_image); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                <div class="header-title">
                    <img src="/skin/default/images/chedui-icon.jpg" height="15"><span><?php echo $type['classname'] ?>车队风采</span>
                </div>
                <div class="list">
                    <div class="list-content">
                        <?php
                            $array = array(',|','|');
                            $image = str_replace($array,'',$v['bus_images']);
                            $image = explode('-',$image);
                        ?>
