    ...
    <nav class="navbar">
        <ul class="nav navbar-nav">
            ...
            <div class="content">
                ...
                <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,gywm_desc from s_gywm a left join s_gywm_field b on (a.id=b.aid) where isshow=1 and tid in(".$type["tid_leafid"].")  order by id asc ");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,gywm); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                <div class="list">
