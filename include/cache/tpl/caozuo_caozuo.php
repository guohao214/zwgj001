...
<html lang="zh-CN">
    <head>
        ...
        <![endif]-->
         ...
         <link href="/skin/default/css/caozuo.css" rel="stylesheet">
    </head>
    <body>
    ...
    <nav class="navbar">
        <ul class="nav navbar-nav">
            ...
            <div class="content">
                <div class="banner">
                	<?php $vn=0;$tablev=syClass("syModel")->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow,body from s_classtype where  tid='26'  order by orders desc,tid limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
                    <img src="<?php echo $v['litpic'] ?>" />
                  <?php } ?>
                </div>
                <div class="header-title">
                    <img src="/skin/default/images/oprate-icon.jpg" height="15"> <span><?php echo $type['classname'] ?></span>
                    <div class="tab-navigate">
                        <ul class="nav nav-tabs" role="tablist">
                            <?php $i=1; ?>
                            <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,lc_image,image_title from s_caozuo a left join s_caozuo_field b on (a.id=b.aid) where isshow=1 and tid in(26)  order by id asc ");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,caozuo); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                                <li class="<?php if ($v['id'] == $channel['id'] || (!isset($channel['id']) && $i==1) ) {?> active <?php } ?>" role="presentation">
                                    <a href="<?php echo $v['url'] ?>"><?php echo $v['title'] ?></a>
                                </li>
                                <?php 
                                    if (!isset($channel['id']) && $i==1) {
                                        $channel = $v;
                                    }
                                ?>
                                <?php $i++; ?>
                ...
                <div class="caozuo-body">
                    <div class="left-title">
                        <?php echo $channel['image_title'] ?>
                    </div>
                    <div class="caozuo-img">
                        <img src="<?php echo $channel['lc_image'] ?>">
                    </div>
                </div>
