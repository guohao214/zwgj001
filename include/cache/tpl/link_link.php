...
<html lang="zh-CN">
    <head>
        ...
        <![endif]-->
         ...
         <link href="/skin/default/css/link.css" rel="stylesheet">
    </head>
    <body>
            ...
            <div class="content">
                ...
                <div class="caozuo-body">
                    <div class="link-content">
                        <?php $vn=0;$tablev=syClass("syModel")->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,orders,mrank,mgold,isshow,keywords,description,link from s_link a left join s_link_field b on (a.id=b.aid) where isshow=1 and tid in(".$type["tid_leafid"].")  order by id asc ");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("channel",$v,0,0,link); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
                            <div class="link"><a href="<?php echo $v['link'] ?>" target="_blank" ><?php echo $v['title'] ?></a></div>
                        <?php } ?>
                    </div>
                </div>
